...
export function MascotasPage() {
  ...
  const endpoint = isVet ? '/mascotas' : '/propietarios/mascotas';
  const { data: mascotasData, isLoading, refetch } = useFetch<MascotasResponse | Mascota[]>(endpoint);
  ...
  return (
    <div className="page">
      <div className="page-header">
        <div>
          <h2 className="page-title">{isVet ? 'Pacientes' : 'Mis Mascotas'}</h2>
          <p className="page-subtitle">
            {isVet ? 'Gestión de pacientes de la clínica' : 'Tus mascotas registradas'}
          </p>
        </div>
      ...
      {showCreate && (
        <CreateMascotaModal
          isVet={isVet}
          userId={isVet ? undefined : user?.proId}
...
function CreateMascotaModal({ isVet, userId, onClose, onCreated }: CreateMascotaModalProps) {
  const { user } = useAuth();
  ...
  const [propietarioId, setPropietarioId] = useState(userId || '');
  ...
  return (
    <Modal
      isOpen
      onClose={onClose}
      title={isVet ? 'Registrar Paciente' : 'Agregar Mascota'}
      footer={
        <>
